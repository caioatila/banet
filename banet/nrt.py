# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/04b_nrt.ipynb.

# %% auto 0
__all__ = ['mask2nan', 'nan2zero', 'RunManager']
...
class RunManager():
    ...
    def update_hotspots(self, location='South_America', mode='7d', save=True):
        """Update hotspots file with new data.
            location is according to the data url naming format
            mode can be on of: 24h, 48h, 7d"""
        url0 = f'https://firms.modaps.eosdis.nasa.gov/' \
            f'data/active_fire/modis-c6.1/csv/MODIS_C6_1_{location}_{mode}.csv'
        ...
        url3 = f'https://firms.modaps.eosdis.nasa.gov/' \
            f'data/active_fire/noaa-21-viirs-c2/csv/J2_VIIRS_C2_{location}_{mode}.csv'
        files = self.path.hotspots.ls(include=['.csv', f'hotspots{self.region}'])
        frp = [pd.read_csv(f) for f in files]
        frp = pd.concat([*frp, *[pd.read_csv(url) for url in [url0, url1, url2, url3]]], axis=0, 
    ...
    def preprocess_dataset(self, max_workers=1, replace=False):
        if self.product == 'VIIRS375':
            ...
            bfilter = BandsFilter(['Red', 'NIR', 'MIR'])
            act_fires = ActiveFiresLog(f'{self.path.hotspots}/hotspots{self.region}.csv')
            viirs.process_all(proc_funcs=[mask2nan, merge_tiles, mir_calc, 
                                          rename2, bfilter, act_fires, nan2zero], 
                              max_workers=max_workers, replace=replace)
